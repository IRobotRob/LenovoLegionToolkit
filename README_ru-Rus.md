...
Lenovo Legion Toolkit создан для ноутбуков Lenovo Legion, а также Legion Go, IdeaPad Gaming, LOQ, Lenovo Slim и ThinkBook, включая их китайские варианты (R-серия, Y-серия).

Поддерживаются поколения 6 (2021), 7 (2022), 8 (2023), 9 (2024), 10 (2025) и 11 (2026), хотя некоторые функции также работают на 5-м поколении (2020). Проблемы, связанные с устройствами старше 6-го поколения или не являющимися Legion, выходят за рамки этого проекта.
...
- Завершение всех процессов, работающих на dGPU (этот, кажется, работает лучше).
- Отключение dGPU на короткое время, что заставит все процессы переместиться на интегрированную GPU.
...
- `LLT_POWER_MODE=<value>`, где `value` — одно из: `1` — Quiet, `2` — Balance, `3` — Performance, `223` — Extreme, `255` — Custom
- `LLT_POWER_MODE_NAME=<value>`, где `value` — одно из: `QUIET`, `BALANCE`, `PERFORMANCE`, `EXTREME`, `CUSTOM`
...
**Когда изменён Hybrid Mode:**
- `LLT_HYBRID_MODE=<value>`, где `value` — одно из: `1` — dGPU Only, `2` — Hybrid, `3` — Hybrid Auto, `4` — Hybrid IGPU Only, `5` — UMA
- `LLT_HYBRID_MODE_NAME=<value>`, где `value` — одно из: `DGPU Only`, `Hybrid`, `Hybrid Auto`, `Hybrid IGPU`, `UMA`

**При подключении / отключении устройства**
- `LLT_DEVICE_CONNECTED=TRUE` / `LLT_DEVICE_CONNECTED=FALSE`
- `LLT_DEVICE_INSTANCE_IDS=<value>`, где `value` — разделённый запятыми список ID экземпляров устройства

**Когда пользователь становится активным / неактивным**
- `LLT_IS_USER_ACTIVE=TRUE` / `LLT_IS_USER_ACTIVE=FALSE`

**Когда сеанс заблокирован / разблокирован**
...
- `llt rgb set <preset>` — установить 4-зонную RGB в `<preset>`
</details>
...
- Приложение Lenovo Vantage
- Служба Lenovo Vantage Service
...
Для обеспечения подлинности исполняемый файл установщика и сам `Lenovo Legion Toolkit.exe` подписаны сертификатом проекта. Эта подпись позволяет собственному обновлению приложения проверить, что установщик не был изменён и произошёл из официального процесса сборки.
...
Вы можете настроить горячие клавиши в настройках LLT.
...
**Почему я не могу переключиться на режим питания Performance, Extreme или Custom от аккумулятора?**
...
Режим обнаружения игр можно настроить в настройках LLT. Есть четыре варианта:
- **Auto (по умолчанию)** — комбинирует все методы ниже
- **Discrete GPU** — реагирует, когда любой процесс начинает использовать дискретную GPU
- **Game Config Store** — реагирует на игры, зарегистрированные в Windows через Xbox Game Bar (`HKEY_CURRENT_USER\System\GameConfigStore\Children`). Вы можете отметить любую программу как игру в настройках Xbox Game Bar (Win+G)
- **Game Mode** — реагирует, когда Windows активирует Игровой режим для процесса на переднем плане

Если игра не обнаруживается, проверьте, какой режим обнаружения настроен в LLT, и убедитесь, что игра соответствует критериям для этого режима.
...
- Зайдите в BIOS и попробуйте найти опцию, похожую на «Enabled GPU Overclocking», и отключите её, запустите Windows и переключите опцию BIOS обратно на Enabled.
- Запустите Windows в безопасном режиме и удалите файл `gpu_oc.json` из папки настроек LLT, которая находится в `"%LOCALAPPDATA%\LenovoLegionToolkit"`.
...
- `--enable-lamp-array` — включает поддержку Lamp Array (Windows Dynamic Lighting) для совместимых клавиатур
...
> [!IMPORTANT] ❗ Важно
> Если ваш ноутбук не является частью серии Legion, Legion Go, IdeaPad Gaming, LOQ, Lenovo Slim или ThinkBook, это программное обеспечение не для вас. Не открывайте запросы о совместимости для других устройств.
